...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    fc1 = X.dot(W1) + b1            # N * H
    relu = np.maximum(0, fc1)       # N * H
    fc2 = relu.dot(W2) + b2         # N * C
    scores = fc2
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    f = scores
    f = (f.T - np.max(f.T, axis=0)).T # N * C, for numeric stability

    arange_N = np.arange(N)

    efsum = np.sum(np.exp(f), axis=1) # N * 1
    loss = np.sum(-f[arange_N, y] + np.log(efsum)) / N # Li = -fyi + log(sum(e^fj))
    loss += reg * ((W1 * W1).sum() + (W2 * W2).sum()) # L2 regularization loss
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    d_f = (np.exp(f).T / efsum).T # N * C, gradient of np.log(efsum)
    d_f[arange_N, y] -= 1         # gradient of -f[arange, y]
    d_f /= N                      # / N

    d_fc2 = d_f                     # N * C, just for numeric stability

    d_relu = d_fc2.dot(W2.T)        # N * H
    d_fc1 = d_relu * (fc1 >= 0)     # N * H, gradient of relu

    dW2 = relu.T.dot(d_fc2)         # H * C
    dW2 += reg * 2 * W2             # regularization
    db2 = np.sum(d_fc2, axis=0)     # C * 1

    dW1 = X.T.dot(d_fc1)            # N * H
    dW1 += reg * 2 * W1             # regularization
    db1 = np.sum(d_fc1, axis=0)     # H * 1
